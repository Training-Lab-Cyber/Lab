...
def create_agent_policy(agent_policy_data, kibana_url, kibana_username, kibana_password):
    ...
    response = requests.post(url, json=agent_policy_data, auth=(kibana_username, kibana_password), headers=headers)
    
    if response.status_code == 200:
        policy_id = response.json().get('item', {}).get('id')
        print(f"Agent Policy '{agent_policy_data['name']}' created successfully with ID: {policy_id}")
        return policy_id
        ...
        if error_message:
            ...
            match = re.search(r"Agent Policy '([a-f0-9-]+)' already exists", error_message)
            if match:
                existing_policy_id = match.group(1)
                print(f"Agent Policy already exists with ID: {existing_policy_id}. Continuing with existing policy.")
...
def create_package_policy(package_policy_data, kibana_url, kibana_username, kibana_password):
    ...
    response = requests.post(url, json=package_policy_data, auth=(kibana_username, kibana_password), headers=headers)
    
    if response.status_code == 200:
        print(f"Package Policy '{package_policy_data['name']}' created successfully.")
    else:
        print(f"Failed to create Package Policy: {response.status_code}, {response.text}")

# Parse command-line arguments
def parse_arguments():
    parser = argparse.ArgumentParser(description="Create Agent and Package Policies in Kibana")
    parser.add_argument('--path', required=True, help="Path to the directory containing AgentPolicy.json and packagepolicies")
...
def main():
    ...
    # Create agent policy
    agent_policy_id = create_agent_policy(agent_policy_data, kibana_url, kibana_username, kibana_password)
    ...
    for package_file in os.listdir(package_policies_dir):
        ...
        create_package_policy(package_policy_data, kibana_url, kibana_username, kibana_password)
